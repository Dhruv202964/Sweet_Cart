  ...
  try {
    const result = await db.query(`
      SELECT 
        ...
        o.delivery_area, o.delivery_city,
...
exports.getDashboardStats = async (req, res) => {
  try {
    const revenueResult = await db.query("SELECT SUM(total_amount) AS total_revenue FROM orders WHERE status = 'Delivered'");
    const pendingResult = await db.query("SELECT COUNT(*) AS pending_count FROM orders WHERE status = 'Pending' OR status = 'Processing'");

    res.json({
      total_revenue: revenueResult.rows[0].total_revenue || 0,
      pending_orders: parseInt(pendingResult.rows[0].pending_count) || 0
    });
  } catch (err) {
...
// 5. Update Order Status (Restored Function)
exports.updateOrderStatus = async (req, res) => {
  try {
    const { order_id, status } = req.body;
    const result = await db.query("UPDATE orders SET status = $1 WHERE order_id = $2 RETURNING *", [status, order_id]);
